...
import { Activity, BrainCircuit, BarChart3, LayoutDashboard, Loader2, Target, ShieldCheck } from 'lucide-react';
...
const DashboardAnalyticsView = ({ mission }) => {
  ...
  if (loading || !stats) return (
    <div className="flex flex-col items-center justify-center p-20 text-[#00d4ff] bg-[#0a0e1a] h-screen">
        ...
        <span className="font-black tracking-[0.3em] uppercase text-xs">Chargement du Système v5.0...</span>
    </div>
  );
  ...
  return (
    <div className="max-w-7xl mx-auto pb-20 animate-in fade-in duration-700">
      ...
      {/* HEADER NOIR ET CHIC */}
      <header className="flex justify-between items-center mb-10 bg-[#111827] p-8 rounded-[40px] border border-slate-800 shadow-2xl relative overflow-hidden">
        <div className="relative z-10">
          ...
          <p className="text-slate-500 text-[10px] font-bold uppercase tracking-[0.4em] mt-2">Intelligence Artificielle d'Audit Légal</p>
        </div>
        <div className="text-right relative z-10">
            <div className="bg-[#00ff88]/10 text-[#00ff88] px-4 py-1 rounded-full text-[10px] font-black border border-[#00ff88]/20 inline-block mb-2">DOSSIER CERTIFIÉ</div>
      ...
      {/* NAVIGATION TABS PRO */}
      <div className="flex bg-[#111827] p-1.5 rounded-2xl mb-10 w-fit border border-slate-800 mx-auto">
        {[
          {id: 'STATS', label: 'Statistiques Globales', icon: BarChart3},
          {id: 'CARTO', label: 'Cartographie des Risques', icon: Target},
          {id: 'PERF', label: 'Indicateurs IA', icon: BrainCircuit}
        ].map(tab => (
          <button 
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            className={`flex items-center gap-3 px-8 py-3 rounded-xl text-[11px] font-black uppercase transition-all ${activeTab === tab.id ? 'bg-[#1f2a40] text-[#00d4ff] shadow-xl border border-slate-700' : 'text-slate-500 hover:text-slate-300'}`}
          >
      ...
      {activeTab === 'STATS' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in slide-in-from-bottom-5">
           <div className="lg:col-span-2 bg-[#111827] p-8 rounded-[40px] border border-slate-800 shadow-sm">
              <h3 className="font-black text-white text-[11px] uppercase mb-10 flex items-center gap-2 tracking-widest text-[#00d4ff]">
                Analyse de Fréquence des Erreurs par Cycle
              </h3>
              <Plot
                data={[
           ...
           <div className="space-y-6">
              <div className="bg-gradient-to-br from-[#111827] to-[#0a0e1a] p-8 rounded-[40px] border border-slate-800">
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Seuil ISA 320</p>
              ...
              <div className="bg-[#111827] p-8 rounded-[40px] border border-slate-800">
                <p className="text-[10px] font-black text-[#ff4f4f] uppercase tracking-widest">Impact Détecté</p>
                <p className="text-4xl font-black text-white mt-2">{stats.impact_total?.toLocaleString()} €</p>
                <div className="mt-4 h-2 bg-slate-800 rounded-full overflow-hidden">
                    <div className="h-full bg-[#ff4f4f]" style={{width: `${Math.min(100, (stats.impact_total/mission.seuil_signification)*100)}%`}}></div>
                </div>
              </div>
           </div>
        </div>
      ...
      {activeTab === 'CARTO' && (
        <div className="bg-[#111827] p-10 rounded-[50px] border border-slate-800 shadow-2xl animate-in zoom-in">
           <Plot
            ...
            layout={{ 
                xaxis: { title: 'Impact Financier (1-5)', range: [0, 6], gridcolor: '#1f2a40' },
      ...
      {activeTab === 'PERF' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in slide-in-from-right-5">
           <div className="bg-[#111827] p-8 rounded-[40px] border border-slate-800 shadow-2xl">
              <h3 className="font-black text-[#00ff88] text-[11px] uppercase mb-10 tracking-widest text-center">Fiabilité du Moteur ML</h3>
              <Plot
                data={[{
                ...
                layout={{ polar: { bgcolor: 'rgba(0,0,0,0)', radialaxis: { visible: true, range: [0, 100], gridcolor: '#1f2a40' } }, height: 400, paper_bgcolor: 'rgba(0,0,0,0)' }}
                style={{ width: "100%" }}
              />
           </div>
           <div className="bg-[#0d1321] p-10 rounded-[40px] border border-slate-800 flex flex-col justify-center text-center">
              <ShieldCheck className="text-[#00d4ff] mx-auto mb-6" size={60} />
              <h4 className="text-2xl font-black text-white italic">Scoring Certifié</h4>
              <p className="text-slate-500 text-xs mt-4 leading-relaxed uppercase tracking-widest">Modèle Random Forest v5.0 de niveau bancaire.</p>
           </div>
        </div>
      )}
    </div>
